...
import 'package:hipax_log/RecepcaoInterno/qr_scan_page.dart';
...
class _SliderUpWidgetState extends State<SliderUpWidget> {
  ...
  Widget _panel(ScrollController sc) {
    ...
    Widget checarStatusVeiculoBotaoViagem() {
      if (transferpainel.classificacaoVeiculo == "INTERNO IDA") {
      ...
      if (transferpainel.classificacaoVeiculo == "INTERNO VOLTA") {
        if (transferpainel.status == 'Programado') {
          return Expanded(
            child: GestureDetector(
              onTap: () {
        ...
        if (transferpainel.status == 'Finalizado' &&
          ...
          return Column(
            children: <Widget>[
              ...
              RatingBar.builder(
                ignoreGestures: true,
                initialRating: transferpainel.notaAvaliacao?.toDouble() ?? 0,
                glowColor: Colors.grey.shade600,
                itemPadding: const EdgeInsets.symmetric(horizontal: 0),
                itemSize: 23,
                allowHalfRating: true,
