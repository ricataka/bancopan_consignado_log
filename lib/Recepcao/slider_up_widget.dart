...
import 'package:hipax_log/Recepcao/qr_scan_page.dart';
...
class _SliderUpWidgetState extends State<SliderUpWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (transfer.uid == '') {
      ...
      List<Participantes> listAusentein = participantesTransfer
          .where(
              (o) => o.isEmbarque == false && o.uidTransferIn == transfer.uid)
          .toList();
      List<Participantes> listEmbarquein = participantesTransfer
          .where((o) => o.isEmbarque == true && o.uidTransferIn == transfer.uid)
          .toList();

      listAusentein.sort((a, b) => a.nome.compareTo(b.nome));
      listEmbarquein.sort((a, b) => a.nome.compareTo(b.nome));
      return transfer.isAvaliado == true
          ? SlidingUpPanel(
              ...
              maxHeight: _panelHeightOpen ?? 0.0,
  ...
  Widget _panel(ScrollController sc) {
    final transferpainel = Provider.of<TransferIn>(context);
        ...
        TextButton(
          onPressed: () {
            Duration valorPrevisaoGoogle =
                Duration(seconds: transferpainel.previsaoChegadaGoogle ?? 0);
            ...
            DatabaseServiceNotificacoes().setNotificacoes(
                ...
                user.email,
                transferpainel.horaInicioViagem ?? Timestamp.now(),
                false);
            DatabaseServiceTransferIn().updateStatuCarroEmTransito(
                transferpainel.uid ?? '', user.email);
      ...
      actions: <Widget>[
        ...
        TextButton(
          onPressed: () {
            DatabaseServiceNotificacoes().setNotificacoes(
                'Veículo ${transferpainel.veiculoNumeracao} ${transferpainel.classificacaoVeiculo} iniciou viagem',
                'Saída de ${transferpainel.origem} com destino a ${transferpainel.destino} com previsão de chegada às ${formatDate(DateTime.now(), [
                      HH,
                      ':',
                      nn
                    ])}',
            ...
            Navigator.pop(context);
            DatabaseServiceTransferIn().updateStatuCarroEmFinalizado(
                transferpainel.uid ?? '', user.email);
            Navigator.pop(context);
        ...
        TextButton(
          onPressed: () {
            DatabaseServiceTransferIn()
                .updateZerarViagem(transferpainel.uid ?? '');

            StatusAlert.show(
              context,
              duration: const Duration(milliseconds: 1500),
              title: 'Transfer reiniciado',
    ...
    Widget checarStatusVeiculoBotaoViagem() {
      if (transferpainel.classificacaoVeiculo == "IN") {
              ...
              Row(
                children: [
                  RatingBar.builder(
                    ignoreGestures: true,
                    initialRating:
                        transferpainel.notaAvaliacao?.toDouble() ?? 0.0,
      ...
      if (transferpainel.classificacaoVeiculo == "OUT") {
        if (transferpainel.status == 'Programado') {
          return Expanded(
            child: GestureDetector(
              onTap: () {
        ...
        if (transferpainel.status == 'Finalizado' &&
          ...
          return Column(
            children: <Widget>[
              Text(
                'Avaliação veículo : ${transferpainel.notaAvaliacao}',
                style: GoogleFonts.lato(
                  fontSize: 16,
                  color: Colors.black87,
                  fontWeight: FontWeight.w700,
